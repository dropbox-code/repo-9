...
describe NetSuite::Records::CheckExpense do
  let(:expense) { described_class.new }

  it 'has the right fields' do
    [
      :amount, :gross_amt, :is_billable, :line, :memo, :tax_1_amt, :tax_rate_1, :tax_rate_2
    ].each do |field|
  ...
  it 'has the right record_refs' do
    [
      :account, :category, :klass, :customer, :department, :location, :tax_code
    ].each do |record_ref|
  ...
  it 'can initialize from a record' do
    record = NetSuite::Records::CheckExpense.new(:amount => 123)
    expense = NetSuite::Records::CheckExpense.new(record)
    expect(expense).to be_kind_of(NetSuite::Records::CheckExpense)
  ...
  describe '#record_type' do
    it 'returns a string of the SOAP record type' do
      expect(expense.record_type).to eql('tranBank:CheckExpense')
    end
  end
end
